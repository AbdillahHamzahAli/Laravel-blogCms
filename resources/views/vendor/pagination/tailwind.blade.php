...
@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation"
        ...
        <div class="flex lg:flex-row sm:flex-1 sm:flex items-center sm:justify-between">
            ...
            <span class="relative z-0 inline-flex ml-2 mt-2 shadow-sm lg:mt-0">
                        ...
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <span aria-current="page">
                                    <span
                                        class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-white bg-black border border-gray-300 dark:text-gray-400 dark:border-secondary-600 dark:bg-secondary-700">{{ $page }}</span>
                                </span>
                            @else
