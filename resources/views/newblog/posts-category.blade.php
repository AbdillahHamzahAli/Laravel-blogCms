            ...
            <div class="w-full">
                ...
                <div class="justify-start my-2 text-start">
                    <a href="{{ route('blog.posts.category', ['slug' => $categoryRoot->slug]) }}"
                        class="my-2 mx-1 inline-block rounded-sm {{ $categoryRoot->title == $category->title ? 'bg-black text-white' : 'hover:text-white hover:bg-black hover:scale-110' }} border-black border-2 px-6 pb-2 pt-2.5 text-xs font-medium uppercase  transition duration-150 ease-in-out">{{ $categoryRoot->title }}</a>
                    @foreach ($categoryRoot->descendants as $item)
